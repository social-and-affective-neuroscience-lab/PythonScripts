...
jitterArray = [1.16, 2.5, 3.30, 4.26, 8.94, 1.1, 0.96, 2.8, 3.5, 3.0, 1.1, 1.5, 2.13, 2.145, 6.18, 9.04]
...
#    jitterArray.remove(timeVar)
    thisExp.addData("ISI Duration", timeVar)
    thisExp.addData('isi Onset', isiOnset)
...
def itiFunc():
    ...
    thisExp.addData('iti Onset', itiOnset)
...
def cueScreenFunc(bucketCue):
    ...
    thisExp.addData('Cue Offset', globalClock.getTime())
...
def trialInstFunc(text):
    ...
    frameN = -1
    interRunOnset=globalClock.getTime()
    while continueRoutine:
    ...
    thisExp.addData('Inter-Run Text Onset', interRunOnset)
    thisExp.addData('Inter-Run Text Offset', globalClock.getTime())
    thisExp.nextEntry()
    routineTimer.reset()
...
# add back in button_resp as a component
    trialComponents = [andSignLeft, andSignRight, button_resp, regCue, selectedVidImgLeft, selectedVidImgRight, selectedTipImgLeft, selectedTipImgRight, topLeftText, topLeftTime, topRightText, topRightTime, botLeftText, botLeftTime, botRightText, botRightTime, blank]
    for thisComponent in trialComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
    ...
    trialClock.reset(-_timeToFirstFrame)
    frameN = -1
    allStimOnset=globalClock.getTime()
    # begin trial
    while continueRoutine and routineTimer.getTime() > 0:
...
#    thisExp.addData('Choice Onset', choiceOnset)
#    thisExp.addData('choice Offset', globalClock.getTime())
    ...
    thisExp.addData("All Stimuli Onset", allStimOnset)
    thisExp.addData('All Stimuli Offset', globalClock.getTime())
    routineTimer.reset()


MainLoop = data.TrialHandler(nReps=1.0, method='sequential',
