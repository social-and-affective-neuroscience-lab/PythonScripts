...
bucketAll = ["a","b","c","d","e", "f", "g", "h", "i","j","k","l", "m","n","o","p","q","r","s", "t", "u", "v", "w", "x", "y", "z", "za", "zb", "zc", "zd",
...
#    routineTimer.add(timeVar)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if text.status == STARTED:
            if tThisFlipGlobal > text.tStartRefresh + timeVar-frameTolerance: # change 2 to timeVar when using jitter
                text.tStop = t
...
def cueScreenFunc(bucketCue):
    ...
    for thisComponent in CueScreenComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Cuetext.started', cueOnset)
...
regCueStr = []
def getRandomFromBucket(bucket):
...
# add back in button_resp as a component
    ...
    trialClock.reset(-_timeToFirstFrame)
    frameN = -1
    allStimuliOnset = globalClock.getTime()
    # begin trial
    while continueRoutine and routineTimer.getTime() > 0:
...
#    lineStim.setAutoDraw(False)
    thisExp.addData("All Stimuli Onset", allStimuliOnset)
...
for thisMainLoop in MainLoop:
    ...
    trialInstFunc(text1)
    itiFunc()
    bucketA = ["a","b","c","d","e", "f", "g", "h", "i","j","k","l", "m","n","o","p","q","r","s", "t", "u", "v", "w", "x", "y", "z", "za", "zb", "zc", "zd"]
...
#            isiFunc()
            getRandomFromBucket(bucketAll)
            getRandomFromBucket(bucketA)
            print(bucketA)
